...
pluginParser' = do
  ...
  return res
  where
    restOfFile = manyTill anySingle eof >> return []
...
plugin =
  symbol "plugin" >> (Plugin <$> stringLiteral <*> option [] exprsInParens)
...
-- a property name
data Path
  = FieldPath BindingForm
              Field -- example: x.val
  | PropertyPath BindingForm
                 Field
                 Property -- example: x.shape.center
...
data Expr
  ...
  | ListAccess Path
               Integer
  | Ctor String
         [PropertyDecl] -- Shouldn't be using this, since we have PropertyDict
  | Layering Path
             Path -- ^ first GPI is *below* the second GPI
  | PluginAccess String
                 Expr
                 Expr -- ^ Plugin name, Substance name, Key
  | ThenOp Expr
           Expr -- COMBAK: double check how transforms are modeled, probably just a list of CompApp
  deriving (Show, Eq, Typeable)
...
selector = do
  ...
  return Selector {selHead = hd, selWith = wi, selWhere = wh, selNamespace = ns}
  where
    wth = fmap concat $ rword "with" *> declPattern `sepEndBy1` semi' <* scn
    whr = rword "where" *> relationPattern `sepEndBy1` semi' <* scn
    namespace = rword "as" >> identifier
    withAndWhere =
      runPermutation $
      (,) <$> toPermutationWithDefault [] wth <*>
      toPermutationWithDefault [] whr
...
relationPattern = try pre <|> bind
  where
    bind = RelBind <$> bindingForm <*> (def >> selectorExpr)
...
selectorExpr = try selectorValConsOrFunc <|> SEBind <$> bindingForm
...
selectorValConsOrFunc = do
  ...
  let env =
        fromMaybe
          (error "Style parser: variable environment is not intiialized.")
          e
  ...
  where
    styleErr s = customFailure (StyleError s)
...
aOperators
        -- Highest precedence
 =
  [ [Prefix (UOp UMinus <$ symbol "-"), Prefix (UOp UPlus <$ symbol "+")]
  , [InfixL (BinOp Exp <$ symbol "^")]
  , [InfixL (BinOp Multiply <$ symbol "*"), InfixL (BinOp Divide <$ symbol "/")]
  , [InfixL (BinOp BPlus <$ symbol "+"), InfixL (BinOp BMinus <$ symbol "-")]
        -- Lowest precedence
  ]
...
layeringExpr =
  tryChoice
    [layeringAbove, layeringBelow, layeringAboveKeyword, layeringBelowKeyword]
  where
...
path :: Parser Path
path =
  try (PropertyPath <$> bindingForm <*> dotId <*> dotId) <|>
...
data SelEnv = SelEnv
  { sTypeVarMap :: M.Map BindingForm StyT -- B : |T
  , sErrors     :: [String]
  , skipBlock   :: Bool
  } deriving (Show, Eq, Typeable)
...
mergeEnv varEnv selEnv =
  foldl mergeMapping varEnv (M.assocs $ sTypeVarMap selEnv)
  where
    mergeMapping :: VarEnv -> (BindingForm, StyT) -> VarEnv
               -- G || (x : |T) |-> G
    ...
    mergeMapping varEnv (BStyVar (StyVar' var), styType) =
      varEnv
      {varMap = M.insert (VarConst var) (toSubType styType) (varMap varEnv)}
...
toSubType :: StyT -> T
toSubType (STTypeVar stvar) =
  TTypeVar $
  TypeVar {typeVarName = typeVarNameS stvar, typeVarPos = typeVarPosS stvar}
toSubType (STCtor stcons) =
  TConstr $
  TypeCtorApp
  { nameCons = nameConsS stcons
...
toSubExpr (SEAppFunc f exprs) =
  C.ApplyFunc $ C.Func {C.nameFunc = f, C.argFunc = map toSubExpr exprs}
toSubExpr (SEAppValCons v exprs) =
  C.ApplyValCons $ C.Func {C.nameFunc = v, C.argFunc = map toSubExpr exprs}
...
toSubPred stypred =
  C.Predicate
  { C.predicateName = C.PredicateConst $ predicateName stypred
...
checkDeclPatterns varEnv selEnv decls =
  foldl (checkDeclPattern varEnv) selEnv decls
...
checkSels varEnv prog =
  let selEnvs = map (checkPair varEnv) prog
  in let errors = concatMap sErrors selEnvs
     in if null errors
          then selEnvs
          else error $ intercalate "\n" errors
  where
    checkPair :: VarEnv -> (Header, Block) -> SelEnv
...
mkLocalVarName (blockNum, substNum) =
  localKeyword ++ "_b" ++ show blockNum ++ "_s" ++ show substNum
...
fullSubst selEnv subst =
  let selStyVars =
        map styVarToString $ filter isStyVar $ M.keys $ sTypeVarMap selEnv
      substStyVars = map styVarToString' $ M.keys subst
      res = sort selStyVars == sort substStyVars
  ...
  in trM1
       ...
       res
  where
    styVarToString (BStyVar (StyVar' v)) = v
...
uniqueKeysAndVals subst =
  let (keys, vals) = unzip $ M.toList subst
  in allUnique keys && allUnique vals
  ...
  if vn == localKeyword
    then case lv
        ...
        -- lv = Just (i, j): substituting into block, so local vars matter
           ...
           Nothing ->
             error
               "LOCAL keyword found without a subst/block id. It should not be used in a selector."
...
-- theta(f[E]) = f([theta(E)]
substituteExpr subst (SEAppFunc fname exprs) =
  SEAppFunc fname $ map (substituteExpr subst) exprs
-- theta(v([E])) = v([theta(E)])
substituteExpr subst (SEAppValCons vname exprs) =
  SEAppValCons vname $ map (substituteExpr subst) exprs
...
substitutePred subst pred =
  pred {predicateArgs = map (substitutePredArg subst) $ predicateArgs pred}

-- theta(|S_r) = ...
substituteRel :: Subst -> RelationPattern -> RelationPattern
-- theta(B := E) |-> theta(B) := theta(E)
substituteRel subst (RelBind bvar sExpr) =
  RelBind (substituteBform Nothing subst bvar) (substituteExpr subst sExpr)
-- theta(Q([a]) = Q([theta(a)])
...
substitutePath lv subst path =
  case path of
    FieldPath bVar field ->
      FieldPath (substituteBform (Just lv) subst bVar) field
    PropertyPath bVar field prop ->
      PropertyPath (substituteBform (Just lv) subst bVar) field prop

substituteField :: LocalVarId -> Subst -> PropertyDecl -> PropertyDecl
substituteField lv subst (PropertyDecl field expr) =
  PropertyDecl field $ substituteBlockExpr lv subst expr
...
substituteBlockExpr lv subst expr =
  case expr of
    ...
    BinOp op e1 e2 ->
      BinOp
        op
        (substituteBlockExpr lv subst e1)
        (substituteBlockExpr lv subst e2)
    ...
    Layering path1 path2 ->
      Layering (substitutePath lv subst path1) (substitutePath lv subst path2)
    ...
    PluginAccess pluginName e1 e2 ->
      PluginAccess
        pluginName
        (substituteBlockExpr lv subst e1)
        (substituteBlockExpr lv subst e2)
    Tuple e1 e2 ->
      Tuple (substituteBlockExpr lv subst e1) (substituteBlockExpr lv subst e2)
    ThenOp e1 e2 ->
      ThenOp (substituteBlockExpr lv subst e1) (substituteBlockExpr lv subst e2)

substituteLine :: LocalVarId -> Subst -> Stmt -> Stmt
substituteLine lv subst line =
  case line of
    Assign path expr ->
      Assign (substitutePath lv subst path) (substituteBlockExpr lv subst expr)
    Override path expr ->
      Override
        (substitutePath lv subst path)
        (substituteBlockExpr lv subst expr)
...
substituteBlock (subst, substNum) (block, blockNum) =
  map (substituteLine (blockNum, substNum) subst) block
...
findType typeEnv name =
  case M.lookup name (valConstructors typeEnv) of
    ...
    Nothing ->
      error $
      "name '" ++
      name ++ "' does not exist in Substance val constructor environment"
...
exprsMatch typeEnv (C.ApplyFunc subE) (C.ApplyFunc styE) = subE == styE
...
relMatchesProg typeEnv subEnv subProg rel =
  any (flip (relMatchesLine typeEnv subEnv) rel) subProg

-- Judgment 15. b |- [S] <| [|S_r]
allRelsMatch :: VarEnv -> C.SubEnv -> C.SubProg -> [RelationPattern] -> Bool
allRelsMatch typeEnv subEnv subProg rels =
  all (relMatchesProg typeEnv subEnv subProg) rels
...
couldMatchRels typeEnv rels stmt = any (flip couldMatchARel stmt) rels
  where
    couldMatchARel :: RelationPattern -> C.SubStmt -> Bool
...
-- Folds over [theta]
filterRels ::
     VarEnv -> C.SubEnv -> C.SubProg -> [RelationPattern] -> [Subst] -> [Subst]
...
matchType varEnv (TTypeVar tvar) (STTypeVar stvar) =
  error "no type vars allowed in match"
matchType varEnv (TConstr tvar) (STTypeVar stvar) =
  error "no type vars allowed in match"
matchType varEnv (TTypeVar tvar) (STCtor stctor) =
  error "no type vars allowed in match"
...
matchBvar subVar (BSubVar styVar) =
  if subVar == styVar
    then Just M.empty
    else Nothing
...
matchDeclLine varEnv (C.Decl subT subVar) (PatternDecl' styT bvar) =
  let typesMatched = matchType varEnv subT styT
  in if typesMatched
...
matchDecl varEnv subProg initSubsts decl
 ...
 =
  let newSubsts = map (flip (matchDeclLine varEnv) decl) subProg
  in trM1 ("new substs: " ++ show newSubsts) $
     merge initSubsts (catMaybes newSubsts)
...
matchDecls varEnv subProg decls initSubsts =
  foldl (matchDecl varEnv subProg) initSubsts decls

----- Overall judgments
find_substs_sel ::
     VarEnv -> C.SubEnv -> C.SubProg -> (Header, SelEnv) -> [Subst]
...
find_substs_sel varEnv subEnv subProg (Select sel, selEnv) =
  let decls = selHead sel ++ selWith sel
      ...
      subst_candidates = filter (fullSubst selEnv) rawSubsts
        -- TODO: check validity of subst_candidates (all StyVars have exactly one SubVar)
      filtered_substs =
        trM1 ("candidates: " ++ show subst_candidates) $
...
find_substs_prog ::
     VarEnv -> C.SubEnv -> C.SubProg -> StyProg -> [SelEnv] -> [[Subst]]
find_substs_prog varEnv subEnv subProg styProg selEnvs =
  let sels = map fst styProg
  in let selsWithEnvs = zip sels selEnvs
     in map (find_substs_sel varEnv subEnv subProg) selsWithEnvs
...
data TagExpr a
  = OptEval Expr -- ^ Thunk evaluated at each step of optimization-time
...
mkPropertyDict propertyDecls = foldl addPropertyDecl M.empty propertyDecls
  where
    addPropertyDecl :: PropertyDict a -> PropertyDecl -> PropertyDict a
          -- TODO: check that the same property is not declared twice
    addPropertyDecl dict (PropertyDecl property expr) =
      M.insert property (OptEval expr) dict
...
pathStr (PropertyPath bvar field property) =
  intercalate "." [show bvar, field, property]
  ...
  in case M.lookup name trn of
       Nothing ->
         let err =
               "Err: Sub obj '" ++
               nameStr name ++
               "' has no fields; can't delete field '" ++ field ++ "'"
         in addWarn trans err
       Just fieldDict ->
         if field `M.notMember` fieldDict
           then let warn =
                      "Warn: Sub obj '" ++
                      nameStr name ++ "' already lacks field '" ++ field ++ "'"
  ...
  in case M.lookup name trn of
       Nothing ->
         let err =
               "Err: Sub obj '" ++
               nameStr name ++
               "' has no fields; can't delete path '" ++ path ++ "'"
         in addWarn trans err
       Just fieldDict ->
     ...
     in let warn1 =
              if fieldDict == M.empty && override
                then Just $
                     "Warning: Sub obj '" ++
                     nameStr name ++
                     "' has no fields, but override was declared"
                else Nothing
    ...
    -- TODO: check existing FExpr is overridden by an FExpr and likewise for Ctor of same type (typechecking)
              in let fieldExpr =
                       case texpr of
                         ...
                         _ -> FExpr texpr -- rule Line-Set-Field-Expr
                 in let fieldDict' = M.insert field fieldExpr fieldDict
                        trn' = M.insert name fieldDict' trn
                    in trans
                       { trMap = trn'
                       , warnings =
                           addMaybes (warnings trans) [warn1, warn2, warn3]
                       }
...
addProperty override trans name field property texpr =
  ...
  in case M.lookup name trn of
       Nothing ->
         let err =
               "Error: Sub obj '" ++
               nameStr name ++ "' has no fields; cannot add property"
         in addWarn trans err
       Just fieldDict ->
         case M.lookup field fieldDict of
           Nothing ->
             ...
             case e of
               ...
               res ->
                 let err =
                       ...
                       field ++
                       "'; found expr '" ++
                       show res ++ "'; cannot add property '" ++ property ++ "'"
                 in addWarn trans err
           Just (FGPI ctor properties)
           -- If the field is GPI, check if property already exists and whether it matches the override setting
            ->
             ...
             in let properties' = M.insert property texpr properties
                    ...
                    trn' = M.insert name fieldDict' trn
                in trans
                   {trMap = trn', warnings = addMaybe (warnings trans) warn}
  ...
  case path of
    FieldPath bvar field ->
      let name = trName bvar
          trans' = deleteField trans name field
      in Right trans'
...
addPath override trans path expr =
  case path
    ...
    FieldPath bvar field ->
      let name = trName bvar
          trans' = addField override trans name field expr
      in Right trans'
    -- rule Line-Set-Prop-Expr
    PropertyPath bvar field property ->
      let name = trName bvar
          trans' = addProperty override trans name field property expr
      in Right trans'
...
translateBlock blockWithNum trans substNum =
  let block' = substituteBlock substNum blockWithNum
...
translateSubstsBlock trans substsNum blockWithNum =
  foldM (translateBlock blockWithNum) trans substsNum
...
translatePair varEnv subEnv subProg trans ((Namespace styVar, block), blockNum) =
  ...
  in if null (sErrors selEnv) && null bErrs
       then let subst = M.empty -- is this the correct empty?
            in translateBlock (block, blockNum) trans (subst, 0) -- skip transSubstsBlock; only one subst
...
insertLabels ::
     (Autofloat a, Eq a) => Translation a -> C.LabelMap -> Translation a
  ...
  where
    ...
    insertLabel (Sub s) fieldDict =
      ...
      in case M.lookup s labels of
           ...
           Just Nothing ->
             let fd' = M.insert labelField (toFieldStr "") fieldDict
             in M.filter (not . usesLabelText s) fd'
    ...
    usesLabelText subName (FGPI shapeType properties) =
      let labelForm =
            OptEval (EPath (FieldPath (BSubVar (VarConst subName)) "label"))
          usesLabel =
            case M.lookup "string" properties of
              Nothing   -> False
...
insertNames trans = trans {trMap = M.mapWithKey insertName $ trMap trans}
  where
    insertName :: Name -> FieldDict a -> FieldDict a -- I guess we don't need to insert names for namespaces
    insertName (Sub name) fieldDict =
      let nameField = "name"
      in M.insert nameField (FExpr $ Done $ StrV name) fieldDict
...
evalPluginAccess ::
     (Autofloat a) => StyValMap a -> Translation a -> Translation a
evalPluginAccess valMap trans =
  trans {trMap = M.mapWithKey (evalFieldAccesses valMap) $ trMap trans}
  where
    evalFieldAccesses ::
         (Autofloat a) => StyValMap a -> Name -> FieldDict a -> FieldDict a
    ...
    evalFieldAccess ::
         (Autofloat a) => StyValMap a -> Field -> FieldExpr a -> FieldExpr a
    evalFieldAccess vmap field (FExpr te) = FExpr $ evalTExpr vmap te
    evalFieldAccess vmap field (FGPI stype properties) =
      let properties' = M.mapWithKey (evalPropertyAccess vmap) properties
      in FGPI stype properties'
    evalPropertyAccess ::
         (Autofloat a) => StyValMap a -> Property -> TagExpr a -> TagExpr a
    ...
    evalPluginExpr vmap (CompApp s es) =
      CompApp s $ map (evalPluginExpr vmap) es
    evalPluginExpr vmap (ObjFn s es) = ObjFn s $ map (evalPluginExpr vmap) es
    evalPluginExpr vmap (ConstrFn s es) =
      ConstrFn s $ map (evalPluginExpr vmap) es
    evalPluginExpr vmap (AvoidFn s es) =
      AvoidFn s $ map (evalPluginExpr vmap) es
    evalPluginExpr vmap (BinOp o e1 e2) =
      BinOp o (evalPluginExpr vmap e1) (evalPluginExpr vmap e2)
    evalPluginExpr vmap (UOp o e) = UOp o $ evalPluginExpr vmap e
    evalPluginExpr vmap (List es) = List $ map (evalPluginExpr vmap) es
    evalPluginExpr vmap (Tuple e1 e2) =
      Tuple (evalPluginExpr vmap e1) (evalPluginExpr vmap e2)
    evalPluginExpr vmap (ThenOp e1 e2) =
      ThenOp (evalPluginExpr vmap e1) (evalPluginExpr vmap e2)
    ...
    evalStatic (EPath (FieldPath bvar field)) =
      case lookupField bvar field trans -- TODO: clean up lookupField location
            of
        FExpr (OptEval (StringLit s)) -> s
        FExpr (Done (StrV s)) -> s
        _ ->
          error
            "expected string for plugin accessor key; after looking up field, didn't get string"
    evalStatic (EPath (PropertyPath (BSubVar v) field property)) =
      error "plugin TODO"
    evalStatic e =
      error
        "expected string or path expression for plugin accessor key; did not get a string"
  ...
  in case foldM (translatePair varEnv subEnv subProg) initTrans numberedProg of
       Right trans ->
         let transWithNames = insertNames trans
             ...
             transWithPlugins =
               evalPluginAccess styValMap transWithNamesAndLabels
...
nameAnonStatements p = map (\(h, b) -> (h, nameAnonBlock b)) p
  where
    nameAnonBlock :: Block -> Block
    nameAnonBlock b =
      let (count, res) = foldl nameAnonStatement (0, []) b
      in res
    ...
    nameAnonStatement (i, b) (AnonAssign e) =
      let path =
            FieldPath
              (BStyVar (StyVar' localKeyword))
              (anonKeyword ++ "_" ++ show i)
          stmt = Assign path e
      in (i + 1, b ++ [stmt])
  ...
  M.fromList
    (map
       (\v ->
          ( subName v
          , M.fromList
              (map (\w -> (propertyName w, r2f $ propertyVal w)) (nameVals v))))
       vals)

-- TODO move lookups to utils; this was moved from GenOptProblem
lookupField ::
     (Autofloat a) => BindingForm -> Field -> Translation a -> FieldExpr a
...
shapeType ::
     (Autofloat a) => BindingForm -> Field -> Translation a -> ShapeTypeStr
...
lookupStyVal subName propName vmap =
  case M.lookup subName vmap of
    Nothing ->
      error
        ("plugin accessors in style: for access " ++
         toAccess subName propName ++ ", primary key does not exist")
    Just propDict ->
      case M.lookup propName propDict of
        Nothing ->
          error
            ("plugin accessors in style: for access " ++
             toAccess subName propName ++ ", property does not exist")
        Just val -> val
  where
    toAccess s1 s2 = s1 ++ "." ++ s2
--------------------------------------------------------------------------------
-- Debugging
-- main :: IO ()
-- main = do
