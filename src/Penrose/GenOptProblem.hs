...
import           Penrose.Functions.Constraints (constrFuncDict,
                                                constrSignatures)
...
foldFields f name fieldDict acc =
  let name' = nameStr name -- TODO do we need do anything with Sub vs Gen names?
  in let res = M.foldrWithKey (f name') [] fieldDict
     in res ++ acc
...
insertGPI trans n field t propDict =
  case M.lookup (Sub n) $ trMap trans of
...
lookupFieldWithVarying bvar field trans varyMap =
  case M.lookup (mkPath [bvarToString bvar, field]) varyMap of
    Just varyVal -> FExpr varyVal {-trace "field lookup was vary" $ -}
    Nothing      -> lookupField bvar field trans {-trace "field lookup was not vary" $ -}
...
lookupPropertyWithVarying bvar field property trans varyMap =
  case M.lookup (mkPath [bvarToString bvar, field, property]) varyMap of
    Just varyVal -> varyVal {-trace "property lookup was vary" $ -}
    Nothing      -> lookupProperty bvar field property trans {- trace "property lookup was not vary" $ -}
...
evalShapes limit shapeNames trans varyMap rng =
  let (shapes, trans', rng') =
        foldl' (evalShape limit varyMap) ([], trans, rng) shapeNames
...
genOptProblemAndState trans optConfig
 ...
 =
  ...
  in let uninitializedPaths = findUninitialized trans
     in let shapeNames = findShapeNames trans
    -- sample varying fields
        in let (transInitFields, g') = initFields varyingPaths trans initRng
    -- sample varying vals and instantiate all the non-float base properties of every GPI in the translation
           in let (!transInit, g'') = initShapes transInitFields shapeNames g'
              in let shapeProperties =
                       transInit `seq` findShapesProperties transInit
                 in let (objfns, constrfns) =
                          (toFns . partitionEithers . findObjfnsConstrs)
                            transInit
                    in let (defaultObjFns, defaultConstrs) =
                             (toFns . partitionEithers . findDefaultFns)
                               transInit
                       in let (!objFnsWithDefaults, !constrsWithDefaults) =
                                ( objfns ++ defaultObjFns
                                , constrfns ++ defaultConstrs)
    ...
    -- Evaluate all expressions once to get the initial shapes
                          in let initVaryingMap = M.empty -- No optimization has happened. Sampled varying vals are in transInit
                             in let (initialGPIs, transEvaled, _) =
                                      evalShapes
                                        evalIterRange
                                        (map (mkPath . list2) shapeNames)
                                        transInit
                                        initVaryingMap
                                        g'' -- intentially discarding the new random feed, since we want the computation result to be consistent within one optimization session
                                in let initState =
                                         lookupPaths varyingPaths transEvaled
